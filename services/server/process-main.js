...
const processRequests = async (body, headers) => {
  ...
  // Authenticate
  const isAuthenticated = await Authenticated(headers, body);
...
const processMain = async (body) => {
  ...
  return await createMicroApp(body.Records[0].body, body.Records[0].messageID);
};
...
const processOutcome = async (body, result) => {
  if (result.status === 201) {
    ...
    await sendMail(body.Records[0].body);
  } else {
